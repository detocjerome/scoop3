...
public class ReportJTableForObsData extends ReportJTable {
    ...
    private static final long serialVersionUID = -8451940319014658468L;
     ...
     * @param report
     */
    public ReportJTableForObsData(final ReportJDialog reportJDialog, final CommonViewImpl commonViewImpl,
	    final Report report) {
	...
	if (index >= 0) {
		...
		try {
		    getModel().setValueAt(
			    (caDataErrorMessageItem.getFlagManual() == null) ? null
				    : /*
				       ...
				       */caDataErrorMessageItem.getFlagManual().getStringQCValue(),
			    index, COLUMN_QC_MANUAL);
		    getModel().setValueAt(caDataErrorMessageItem.isErrorChecked(), index, COLUMN_CHECKBOX);
		} catch (final Exception e) {
		    final UnhandledException exception = new UnhandledException("index : " + index
			    + " / caDataErrorMessageItem.getFlagManual() : " + caDataErrorMessageItem.getFlagManual(),
			    e);
		}
	    }
	...
	if (rowVector == null) { // avoid creation of new instance
	    rowVector = new Vector<Object>(NB_COLUMNS_OBSERVATION_DATA);
	} else {
	    rowVector.clear();
	}
	...
	if (caErrorMessageItem.getErrorMessageType() == ERROR_MESSAGE_TYPE.OBSERVATION_DATA) {
	    final CADataErrorMessageItem caDataErrorMessageItem = (CADataErrorMessageItem) caErrorMessageItem;
	    final boolean displayError = (!(commonViewImpl instanceof ReferenceViewImpl)
		    && !caDataErrorMessageItem.isErrorOnReferenceParameter());
	...
	final String[] columnNames = new String[NB_COLUMNS_OBSERVATION_DATA];
