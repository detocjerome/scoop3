...
public class ReportJTableForObsMetadata extends ReportJTable {

    private static final int COLUMN_CHECKBOX = 0;
    private static final int COLUMN_COMMENT = 6;
    private static final int COLUMN_ERROR_MESSAGE = 3;
    private static final int COLUMN_IS_GENERATED_DURING_LAST_CONTROL = 1;
    private static final int COLUMN_OBSERVATION_REF = 2;
    private static final int COLUMN_QC_AUTO = 4;
    private static final int COLUMN_QC_MANUAL = 5;

    private static final long serialVersionUID = 6962402807557664133L;

    /**
     * @param commonViewImpl
     * @param report
     */
    public ReportJTableForObsMetadata(final ReportJDialog reportJDialog, final CommonViewImpl commonViewImpl,
	    final Report report) {
	...
	for (int rowIndex = 0; rowIndex < getModel().getRowCount(); rowIndex++) {
	    if (getModel().getValueAt(rowIndex, COLUMN_OBSERVATION_REF).equals(caErrorMessageItem.getObs1Id())
		    && getModel().getValueAt(rowIndex, COLUMN_ERROR_MESSAGE).equals(caErrorMessageItem.getDetails())) {
		...
		if (caErrorMessageItem instanceof CAMetadataErrorMessageItem) {
		    getModel().setValueAt(
			    ((CAMetadataErrorMessageItem) caErrorMessageItem).isGenerateDuringLastControl(), rowIndex,
			    COLUMN_IS_GENERATED_DURING_LAST_CONTROL);
		} else {
		    getModel().setValueAt(false, rowIndex, COLUMN_IS_GENERATED_DURING_LAST_CONTROL);
		}
	...
	final Object[] row = new Object[NB_COLUMNS_OBSERVATION_METADATA];
	...
	if (caErrorMessageItem instanceof CAMetadataErrorMessageItem) {
	    row[COLUMN_IS_GENERATED_DURING_LAST_CONTROL] = ((CAMetadataErrorMessageItem) caErrorMessageItem)
		    .isGenerateDuringLastControl();
	} else {
	    row[COLUMN_IS_GENERATED_DURING_LAST_CONTROL] = false;
	}
	if (addClasses) {
	    classes.add(Boolean.class);
	}

	row[COLUMN_OBSERVATION_REF] = caErrorMessageItem.getObs1Id();
	if (addClasses) {
	...
	// ? ((caErrorMessageItem.getFlagManual().equals(QCValues.QC_A)
	// || caErrorMessageItem.getFlagManual().equals(QCValues.QC_Q))
	// ? caErrorMessageItem.getFlagManual().toString().substring(
	// caErrorMessageItem.getFlagManual().toString().length() - 1,
	// caErrorMessageItem.getFlagManual().toString().length())
	// : caErrorMessageItem.getFlagManual().getQCValue())
	// : null;
	row[COLUMN_QC_MANUAL] = (caErrorMessageItem.getFlagManual() != null)
		? caErrorMessageItem.getFlagManual().getStringQCValue() : null;
	if (addClasses) {
	    classes.add(Integer.class);
	}
	...
	// Sort by ObsId and Details
	Collections.sort(values, (final Object[] o1, final Object[] o2) -> {
	    int res = 0;
	    try {
		res = o1[COLUMN_OBSERVATION_REF].toString().compareTo(o2[COLUMN_OBSERVATION_REF].toString());
	    } catch (final NullPointerException npe) {
	    }
	    if (res == 0) {
		try {
		    res = o1[COLUMN_ERROR_MESSAGE].toString().compareTo(o2[COLUMN_ERROR_MESSAGE].toString());
		} catch (final NullPointerException npe) {
		}
	    }
	    return res;
	...
	if (caErrorMessageItem.getErrorMessageType() == ERROR_MESSAGE_TYPE.OBSERVATION_METADATA) {
	...
	return new DefaultTableModel() {
		...
		case COLUMN_QC_MANUAL:
		    ...
		    Integer value;
		    if ((String) getValueAt(row, column) != null) {
			value = Integer.valueOf(
				(QCValues.getQCValuesFromString((String) getValueAt(row, column))).getByteQCValue());
		    } else {
			value = null;
		    }
	...
	final String stationId = (String) getValueAt(convertRowIndexToView(row), COLUMN_OBSERVATION_REF);
	...
	for (final CAErrorMessageItem caErrorMessageItem : errorMessages) {
	    if (caErrorMessageItem.getObs1Id().equals(stationId) && caErrorMessageItem.getDetails().equals(errorMsg)) {
	...
	getColumnModel().getColumn(COLUMN_IS_GENERATED_DURING_LAST_CONTROL).setMinWidth(checkboxColumnWidth);
	getColumnModel().getColumn(COLUMN_IS_GENERATED_DURING_LAST_CONTROL).setPreferredWidth(checkboxColumnWidth);
	getColumnModel().getColumn(COLUMN_IS_GENERATED_DURING_LAST_CONTROL).setMaxWidth(checkboxColumnWidth);
